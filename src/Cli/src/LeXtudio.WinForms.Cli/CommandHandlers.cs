...
namespace LeXtudio.WinForms.Cli
{
    public static class CommandHandlers
    {
        public static int RunDoctor(OutputOptions options)
        {
            return WriteResult("doctor", "Validated the WinForms development environment.", options);
        }

        public static int RunVersion(OutputOptions options)
        {
            return WriteResult("version", "LeXtudio.WinForms.Cli version 1.0.0", options);
        }

        public static int RunNew(Queue<string> tokens, OutputOptions options)
        {
            var name = tokens.Count > 0 ? tokens.Dequeue() : "WinFormsApp";
            ...
            if (options.DryRun)
            {
                Console.WriteLine($"[dry-run] create new WinForms app in {targetDirectory}");
                return 0;
            }

            Directory.CreateDirectory(targetDirectory);
            File.WriteAllText(Path.Combine(targetDirectory, $"{name}.csproj"), GetWinFormsProjectFile(name));
            File.WriteAllText(Path.Combine(targetDirectory, "Program.cs"), GetWinFormsProgramCode(name));

            return WriteResult("new", $"Created WinForms app '{name}' in {targetDirectory}", options);
        }
        ...
        public static int RunDevFlow(Queue<string> tokens, OutputOptions options)
        {
            if (tokens.Count == 0)
            {
                return WriteResult("devflow", "Usage: dotnet winflex devflow <status|screenshot|tap|webview> [options]", options);
            }
            ...
            return subcommand switch
            {
                ...
                "help" => WriteResult("devflow", "Usage: dotnet winflex devflow <status|screenshot|tap|webview> [options]", options),
                "--help" => WriteResult("devflow", "Usage: dotnet winflex devflow <status|screenshot|tap|webview> [options]", options),
                "-h" => WriteResult("devflow", "Usage: dotnet winflex devflow <status|screenshot|tap|webview> [options]", options),
        ...
        private static int RunDevFlowStatus(Queue<string> tokens, OutputOptions options)
        {
            ...
            {
                return WriteResult("devflow", $"Unable to contact WinForms DevFlow agent at {host}:{port}: {ex.Message}", options);
            }
            ...
            Console.WriteLine("WinForms DevFlow agent status:");
        ...
        private static int RunDevFlowScreenshot(Queue<string> tokens, OutputOptions options)
        {
            ...
            outputFile ??= "WinForms-devflow-screenshot.png";
        ...
        {
            if (tokens.Count == 0)
                return WriteResult("devflow", "Usage: dotnet winflex devflow webview <contexts|screenshot> [options]", options);
            ...
            return sub switch
            {
                ...
                _ => WriteResult("devflow", "Usage: dotnet winflex devflow webview <contexts|screenshot> [options]", options)
            };
        }
        ...
        private static int RunDevFlowWebViewScreenshot(Queue<string> tokens, OutputOptions options)
        {
            ...
            outputFile ??= "WinForms-devflow-webview-screenshot.png";
        ...
        private static string GetEnableWindowsTargetingArg(string? target)
        {
            ...
            if (content.Contains("<UseWinForms>true</UseWinForms>", StringComparison.OrdinalIgnoreCase)
        ...
        private static string GetWinFormsProjectFile(string name)
        {
        ...
        private static string GetWinFormsProgramCode(string name)
        {
        ...
        private static int UnknownDevFlowSubcommand(string subcommand)
        {
            ...
            Console.Error.WriteLine("Run 'dotnet winflex devflow --help' for available commands.");
            return 1;
        }
