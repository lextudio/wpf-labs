...
namespace LeXtudio.WinForms.Cli
{
    public static class Program
    {
        ...
        private static int UnknownCommand(string command)
        {
            ...
            Console.Error.WriteLine("Run 'dotnet winflex --help' for available commands.");
            return 1;
        }

        private static void ShowHelp()
        {
            Console.WriteLine("LeXtudio.WinForms.Cli - WinForms command line utility");
            Console.WriteLine();
            Console.WriteLine("Usage: dotnet winflex [options] <command> [command-options]");
            Console.WriteLine();
            ...
            Console.WriteLine("  doctor       Validate the WinForms development environment");
            Console.WriteLine("  version      Display CLI and environment version information");
            Console.WriteLine("  new          Scaffold a new WinForms app or library");
            Console.WriteLine("  build        Build a WinForms project with WinForms-aware defaults");
            Console.WriteLine("  run          Run a WinForms application");
            Console.WriteLine("  publish      Publish a WinForms application for deployment");
            Console.WriteLine("  package      Package WinForms output artifacts");
            Console.WriteLine("  diagnostics  Run WinForms-specific diagnostics and validation");
            Console.WriteLine("  env          Inspect installed SDKs and tooling");
            Console.WriteLine("  devflow      Query a running WinForms DevFlow agent and inspect runtime state");
